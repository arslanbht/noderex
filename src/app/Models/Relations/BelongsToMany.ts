 ...
 */
export class BelongsToMany<T extends Model> {
  ...
  public async get(): Promise<T[]> {
    ...
    // Use raw query for many-to-many relationship
    const query = `
    ...
    const results = await relatedRepository.query(query, [parentId]);
  ...
  public async count(): Promise<number> {
    ...
    const result = await relatedRepository.query(query, [parentId]);
  ...
  public async detach(models?: T[] | number[]): Promise<void> {
    ...
    if (models && models.length > 0) {
      ...
      `;
      await relatedRepository.query(query, [parentId, ...ids]);
    } else {
      const query = `
        DELETE FROM ${this.pivotTable}
        WHERE ${this.foreignPivotKey} = ?
      `;
      await relatedRepository.query(query, [parentId]);
    }
  }
  ...
  /**
   * Toggle related models (attach if detached, detach if attached)
   */
